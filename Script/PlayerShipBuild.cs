...
public class PlayerShipBuild : MonoBehaviour
{
    ...
    void BuyItem()
    {
        ...
        {
            print(tmpSelection.transform.parent.gameObject.GetComponent<ShopPiece>().ShopSelection.iconName);
        ...
        bank = bank - 
            int.Parse(tmpSelection.transform.parent.
    ...
    void LackOfCredits()
    {
        if (bank < int.Parse(target.transform.Find("itemText").GetComponent<TextMesh>().text))
        {
            Debug.Log("CAN'T BUY");
        }
    }
